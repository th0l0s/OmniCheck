...
async def _fetch_responses(client, ip, api_key):
    try:
        r = await client.get(f"{NETLAS_BASE}/responses/",
                             params={"q": f"ip:{ip}", "source_type": "include", "start": 0},
                             headers={"X-API-Key": api_key}, timeout=20.0)
        r.raise_for_status()
        return r.json().get("items", [])
    except Exception as exc:
        log.warning("netlas responses %s failed: %s", ip, exc)
        return None
...
async def fetch(cfg, ctx) -> dict:
    ...
    items = await asyncio.gather(*[_fetch_responses(ctx.client, t, api_key) for t in targets])
