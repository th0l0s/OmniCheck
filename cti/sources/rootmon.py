...
No credentials, no dnspython: a real DNS query for the root SOA over UDP
(with EDNS0 NSID option to identify the anycast instance) and a connect-only
TCP handshake, timed per server. Root hints are fetched live from IANA
named.root with a static fallback. SOA serial divergence across instances is
flagged, inspired by RIPE DNSmon methodology.
"""
...
def _build_query():
    ...
    # ARCOUNT=1 for the OPT RR we append (EDNS0 NSID request)
    header = struct.pack("!HHHHHH", txid, 0x0000, 1, 0, 0, 1)
    question = b"\x00" + struct.pack("!HH", QTYPE_SOA, QCLASS_IN)  # qname "."
    # OPT RR: name=\x00 (root), type=41, class=4096 (UDP buf), TTL=0
    # RDATA: NSID option code=3, length=0 (request — server fills it in response)
    opt_rdata = struct.pack("!HH", 3, 0)
    opt_rr = b"\x00" + struct.pack("!HHIH", 41, 4096, 0, len(opt_rdata)) + opt_rdata
    return txid, header + question + opt_rr
...
def _skip_dnsname(data: bytes, off: int) -> int:
    """Advance offset past a DNS-encoded name (follows at most one pointer)."""
    while off < len(data):
        n = data[off]
        if n == 0:
            return off + 1
...
def _extract_nsid(data: bytes) -> str | None:
    """Return the NSID string from the EDNS0 OPT record in additional section."""
    if len(data) < 12:
        return None
    ...
    for _ in range(ar):
        ...
        off += 10
        if rtype == 41:  # OPT
            p, opt_end = off, off + rdlen
            while p + 4 <= opt_end:
                ocode, olen = struct.unpack("!HH", data[p:p + 4])
                p += 4
                if ocode == 3 and p + olen <= opt_end:  # NSID
                    raw = data[p:p + olen]
...
def _extract_serial(data: bytes, ancount: int) -> int | None:
    """Return the SOA serial from the first SOA answer, or None."""
    if ancount < 1 or len(data) < 12:
        return None
    off = 12
    off = _skip_dnsname(data, off)  # skip question qname
    ...
    for _ in range(ancount):
        ...
        if rtype == 6:  # SOA
            rdata_end = off + rdlen
            p = _skip_dnsname(data, off)   # skip mname
            p = _skip_dnsname(data, p)     # skip rname
            if p + 4 <= rdata_end and p + 4 <= len(data):
...
async def _probe_udp(ip, ver, letter, host):
    ...
    try:
        ...
        serial = _extract_serial(data, parsed["ancount"]) if parsed["rcode"] == 0 else None
        nsid = _extract_nsid(data) if parsed["rcode"] == 0 else None
        return {"target": letter, "ip": ip, "ip_version": ver, "protocol": "udp",
                "ok": parsed["rcode"] == 0, "latency_ms": round((time.time() - start) * 1000, 1),
                "rcode": parsed["rcode"], "serial": serial, "nsid": nsid}
...
def parse(raw: dict) -> dict:
    ...
    # SOA serial + NSID — one value per letter from first UDP ok probe
    letter_serial: dict[str, int] = {}
    letter_nsid: dict[str, str] = {}
    for r in results:
        if r.get("protocol") == "udp" and r.get("ok"):
            if r.get("serial") is not None:
                letter_serial.setdefault(r["target"], r["serial"])
            if r.get("nsid") is not None:
                letter_nsid.setdefault(r["target"], r["nsid"])
    serial_counts: dict[int, int] = {}
    for s in letter_serial.values():
        serial_counts[s] = serial_counts.get(s, 0) + 1
    common_serial = max(serial_counts, key=serial_counts.get) if serial_counts else None
    serial_diverge = len(serial_counts) > 1

    rows = []
    for s in raw.get("servers", []):
        srv = [r for r in results if r["target"] == s["letter"]]
        up = sum(1 for r in srv if r.get("ok"))
        ser = letter_serial.get(s["letter"])
        diverged = serial_diverge and ser is not None and ser != common_serial
        nsid = letter_nsid.get(s["letter"])
        rows.append({"server": s["letter"], "hostname": s["hostname"],
                     "ipv4": s["ipv4"], "checks_ok": f"{up}/{len(srv)}",
                     "serial": ser, "nsid": nsid,
                     "status": ("diverged" if diverged else
                                "ok" if up == len(srv) and srv else
                                "warning" if up else "critical"),
                     "url": f"https://root-servers.org/server/{s['letter'].lower()}"})
    return {
