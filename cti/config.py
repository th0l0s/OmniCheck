...
def update_targets(targets: list[str], path: str | Path | None = None) -> None:
    """Rewrite shodan + netlas targets in config.yaml; preserves ${ENV_VAR} refs.
    Note: yaml.safe_dump strips comments — acceptable trade-off for live editing."""
    p = Path(path or os.getenv("CTI_CONFIG") or _DEFAULT_PATH)
    if not p.exists():
        return
    raw = yaml.safe_load(p.read_text(encoding="utf-8")) or {}
    for feed in ("shodan", "netlas"):
        raw.setdefault("sources", {}).setdefault(feed, {})["targets"] = targets
    p.write_text(
